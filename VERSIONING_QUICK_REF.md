...
|------|--------|---------|----------|
| **Development** | `vX.Y.Z-dev.N` | `v2.28.5-dev.1` | Development pushes, testing iterations |
| **Stable** | `vX.Y.Z` | `v2.28.5` | Production-ready releases |
...
./update-version.sh v2.28.5-dev.2
./update-version.sh v2.28.5-dev.3
```

#### Create Stable Release
```bash
# After testing dev versions, create stable release
./update-version.sh v2.28.5
```

### Important Notes

1. **Docker Image Tag**: Always uses the **base version** (e.g., `v2.28.5`)
   - Development versions (`v2.28.5-dev.1`) still use Docker tag `v2.28.5`
   - The `-dev.N` suffix is only for Git tags and GitHub releases

2. **Git Tags**: Use the full version including `-dev.N` for development
   ```bash
   git tag -a v2.28.5-dev.1 -m "Development iteration 1"
   git tag -a v2.28.5 -m "Stable release"
   ```
...
1. Start development:
   ./update-version.sh v2.28.5-dev.1
   → Updates files to use Docker tag v2.28.5
   → Creates Git tag v2.28.5-dev.1
   → Creates GitHub pre-release v2.28.5-dev.1

2. Iterate on testing:
   ./update-version.sh v2.28.5-dev.2
   → Updates files to use Docker tag v2.28.5
   → Creates Git tag v2.28.5-dev.2
   → Creates GitHub pre-release v2.28.5-dev.2

3. Release stable:
   ./update-version.sh v2.28.5
   → Updates files to use Docker tag v2.28.5
   → Creates Git tag v2.28.5
   → Creates GitHub release v2.28.5 (not pre-release)
```

### Version Comparison

Semver comparison rules apply:
- `v2.28.5-dev.1` < `v2.28.5-dev.2` < `v2.28.5`
